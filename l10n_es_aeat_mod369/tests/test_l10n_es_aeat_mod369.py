...
class TestL10nEsAeatMod369Base(TestL10nEsAeatModBase):
    ...
    def test_model_369_amounts(self):
        ...
        for country_code in self.sale_invoices.keys():
            ...
            spain_goods_line_vat_type = sum([line.vat_type for line in spain_goods_line_filter])
            self.assertEqual(country_amount_tax, spain_goods_line_vat_type)
            # checking amount of tax
            spain_goods_line_amount = sum([line.amount for line in spain_goods_line_filter])
    ...
    def test_model_369_amounts_page_5_6(self):
        ...
        num_invoices_from_mod369 = len(self.model369.tax_line_ids.mapped('move_line_ids').mapped('move_id'))
        self.assertEqual(num_invoices_from_test, num_invoices_from_mod369)
        pages_5_6_total = sum(
            self.model369.total_line_ids.mapped('page_5_6_total'))
